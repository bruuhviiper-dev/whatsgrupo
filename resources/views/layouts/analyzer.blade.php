...
<body class="bg-slate-50" x-data="{ mobileMenuOpen: false }">
  ...
  <header class="@yield('navbar_color', 'bg-[#25D366]') sticky top-0 transition-colors duration-300 z-50 shadow-md">
    <div class="max-w-[1400px] mx-auto px-4 py-3 flex items-center justify-between gap-4">
      ...
      <!-- Logo -->
      <a href="{{ route('tools.engagement.create') }}" class="flex items-center gap-2 text-white decoration-none hover:opacity-90 transition-opacity">
        <x-heroicon-s-sparkles class="w-7 h-7 sm:w-8 sm:h-8" />
        <div class="flex flex-col leading-tight">
          <span class="font-black text-base sm:text-lg tracking-tight">ANALISADOR<span class="text-green-100 font-bold">GRUPOS</span></span>
      ...
      <div class="flex items-center gap-2 sm:gap-4">
        ...
        <nav class="hidden md:flex items-center gap-2">
          <a href="{{ route('tools.engagement.create') }}" class="bg-white text-[#25D366] px-4 py-2 rounded-md font-bold text-xs uppercase tracking-wider shadow-sm hover:bg-slate-50 transition-colors flex items-center gap-1.5">
