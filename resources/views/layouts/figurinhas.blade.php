...
<body class="bg-slate-50" x-data="{ mobileMenuOpen: false }">
  ...
  <header class="@yield('navbar_color', 'bg-[#25D366]') sticky top-0 transition-colors duration-300 z-50 shadow-md">
    <div class="max-w-[1400px] mx-auto px-4 py-3 flex items-center justify-between gap-4">
      
      <!-- Logo -->
      <a href="{{ route('figurinhas.index') }}" class="flex items-center gap-2 text-white decoration-none hover:opacity-90 transition-opacity">
        <x-heroicon-s-face-smile class="w-7 h-7 sm:w-8 sm:h-8" />
        <div class="flex flex-col leading-tight">
          <span class="font-black text-base sm:text-lg tracking-tight">FIGURINHAS<span class="text-green-100 font-bold">WHATS</span></span>
      ...
      <div class="flex items-center gap-2 sm:gap-4">
        ...
        <nav class="hidden md:flex items-center gap-2">
          <a href="{{ route('figurinhas.create') }}" class="bg-white text-[#25D366] px-4 py-2 rounded-md font-bold text-xs uppercase tracking-wider shadow-sm hover:bg-slate-50 transition-colors flex items-center gap-1.5">
  ...
  <nav class="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 shadow-2xl md:hidden z-40">
    <div class="flex items-center justify-around h-20 max-w-full px-2">
      ...
      <!-- FAB Enviar Figurinha - Centro destacado -->
      <a href="{{ route('figurinhas.create') }}" class="flex flex-col items-center justify-center -translate-y-4 relative">
        <div class="w-16 h-16 rounded-full bg-gradient-to-br from-[#25D366] to-[#128C7E] shadow-lg shadow-green-500/40 flex items-center justify-center transition-transform hover:scale-110 active:scale-95">
          <x-heroicon-s-arrow-up-tray class="w-8 h-8 text-white" />
        </div>
