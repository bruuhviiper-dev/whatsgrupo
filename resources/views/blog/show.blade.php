...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            <article class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 sm:p-10">
                ...
                <div class="flex items-center justify-between gap-4 mb-4">
                    <span
                        class="px-3 py-1 rounded-full text-[10px] font-bold bg-green-100 text-green-800 uppercase tracking-widest">
                        Dica & Segurança
                    </span>
                ...
                <div class="flex items-center gap-3 pb-6 border-b border-slate-100 mb-6 text-xs text-slate-400">
                    <div
                        class="w-8 h-8 rounded-full bg-gradient-to-br from-green-400 to-emerald-500 text-white flex items-center justify-center font-bold text-sm">
                        W
                    </div>
                    <div>
                ...
                {{-- Conteúdo do Artigo --}}
                <div
                    class="prose prose-slate prose-a:text-primary max-w-none text-slate-600 text-sm sm:text-base leading-relaxed">
                    {!! Str::markdown($post->content) !!}
                </div>
                {{-- Compartilhamento & SEO Call --}}
                <div
                    class="mt-10 pt-6 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        ...
                        <a href="https://api.whatsapp.com/send?text={{ rawurlencode($post->title . ': ' . url()->current()) }}"
                            ...
                            class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-[#25D366] text-white hover:bg-[#1da851] transition-colors shadow-sm"
                            title="WhatsApp">
                        ...
                        <a href="https://t.me/share/url?url={{ urlencode(url()->current()) }}&text={{ rawurlencode($post->title) }}"
                            target="_blank" rel="noopener noreferrer"
                            class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-[#0088cc] text-white hover:bg-[#0077b3] transition-colors shadow-sm"
                            title="Telegram">
                            <svg class="w-5 h-5 fill-current" viewBox="0 0 24 24">
                                <path
                        ...
                        <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}"
                            target="_blank" rel="noopener noreferrer"
                            class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-[#1877F2] text-white hover:bg-[#166fe5] transition-colors shadow-sm"
                            title="Facebook">
                            <svg class="w-5 h-5 fill-current" viewBox="0 0 24 24">
                                <path
                        ...
                        <div x-data="{ copiedIg: false }">
                            <button
                                @click="navigator.clipboard.writeText('{{ url()->current() }}'); copiedIg = true; setTimeout(() => copiedIg = false, 2000)"
                                class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-gradient-to-tr from-[#f09433] via-[#e6683c] to-[#bc1888] text-white hover:opacity-90 transition-opacity shadow-sm"
                        ...
                        <div x-data="{ copied: false }">
                            <button
                                ...
                                class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-100 text-slate-700 hover:bg-slate-200 transition-colors shadow-sm text-xs font-bold"
                                title="Copiar link">
                ...
                <div class="space-y-4" x-data="{ active: null }">
                    <div class="border-b border-slate-50 pb-4">
                        <button @click="active === 1 ? active = null : active = 1"
                            class="w-full flex justify-between items-center text-left font-bold text-slate-800 hover:text-primary transition-colors text-sm sm:text-base">
                            ...
                            <x-heroicon-m-chevron-down class="w-5 h-5 text-slate-400 transition-transform"
                                x-bind:class="active === 1 ? 'rotate-180 text-primary' : ''" />
                        </button>
                        <div x-show="active === 1" x-collapse class="mt-2 text-xs sm:text-sm text-slate-500 leading-relaxed"
                            style="display: none;">
                            Sim! Qualquer administrador pode cadastrar o link oficial de convite do seu grupo ou canal sem
                            custo algum. O cadastro é rápido e traz visitas orgânicas e constantes.
                        </div>
                    </div>
                    ...
                    <div class="border-b border-slate-50 pb-4">
                        <button @click="active === 2 ? active = null : active = 2"
                            ...
                            <x-heroicon-m-chevron-down class="w-5 h-5 text-slate-400 transition-transform"
                                x-bind:class="active === 2 ? 'rotate-180 text-primary' : ''" />
                        </button>
                        <div x-show="active === 2" x-collapse class="mt-2 text-xs sm:text-sm text-slate-500 leading-relaxed"
                            style="display: none;">
                            Nossa equipe realiza moderações constantes ao longo do dia. A aprovação ocorre tipicamente em
                            menos de 4 horas úteis, desde que o grupo atenda aos Termos de Serviço.
                        </div>
                    </div>
                    ...
                    <div class="pb-2">
                        <button @click="active === 3 ? active = null : active = 3"
                            ...
                            <x-heroicon-m-chevron-down class="w-5 h-5 text-slate-400 transition-transform"
                                x-bind:class="active === 3 ? 'rotate-180 text-primary' : ''" />
                        </button>
                        <div x-show="active === 3" x-collapse class="mt-2 text-xs sm:text-sm text-slate-500 leading-relaxed"
                            style="display: none;">
                            Os pacotes VIP são serviços pagos opcionais que colocam o link do seu grupo em posições fixas de
                            destaque no topo do portal. Isso garante tráfego até 10 vezes maior e conversões aceleradas de
                            novos membros.
                        </div>
                    </div>
                </div>
        ...
        <aside class="space-y-6 lg:sticky lg:top-8 h-max">
                ...
                <div class="space-y-4">
                    @foreach($relatedPosts as $related)
                        <a href="/blog/{{ $related->slug }}"
                            class="group block pb-3 border-b border-slate-50 last:border-0 last:pb-0">
                            <span class="text-[10px] text-slate-400 font-bold block mb-0.5">
                                {{ $related->created_at->translatedFormat('d/m/Y') }}
                            </span>
                            <h4
                                class="font-bold text-slate-700 group-hover:text-primary transition-colors text-xs sm:text-sm leading-snug line-clamp-2">
                                {{ $related->title }}
                            </h4>
                        </a>
                    @endforeach
            ...
            {{-- Widget CTA Divulgar --}}
            <div
                class="bg-gradient-to-br from-green-500 to-emerald-700 text-white p-6 rounded-3xl shadow-sm relative overflow-hidden">
                ...
                <a href="{{ route('send-group.create') }}"
                    class="inline-block bg-white text-emerald-800 font-bold text-xs uppercase tracking-wider px-5 py-2.5 rounded-lg shadow-md hover:bg-slate-50 transition-colors relative z-10">
                    ➕ Enviar Grupo Grátis
                </a>
            </div>
                ...
                <div class="flex flex-wrap gap-2">
                    @foreach($blogCategories as $cat)
                        <a href="{{ route('blog.category', $cat->slug) }}"
                            class="px-3 py-1.5 rounded-lg bg-slate-50 border border-slate-100 text-slate-600 hover:bg-green-50 hover:text-[#25D366] hover:border-green-200 transition-colors text-xs font-semibold flex items-center gap-1.5">
                            <x-dynamic-component :component="$cat->icon ?? 'heroicon-o-folder'" class="w-3.5 h-3.5" />
                            {{ $cat->name }}
                        </a>
                    @endforeach
                </div>
