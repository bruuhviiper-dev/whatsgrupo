...
<div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
    ...
    <div class="lg:col-span-6 space-y-8">
        ...
        <div class="rounded-2xl p-6 border border-slate-200 relative overflow-hidden bg-white shadow-sm" style="background: linear-gradient(135deg, rgba(37,211,102,0.08), rgba(0,200,150,0.02));">
            <div class="relative z-10 max-w-3xl flex items-center justify-between gap-4 flex-wrap">
                <div class="flex items-center gap-3">
                    <div class="w-12 h-12 bg-green-50 text-primary rounded-2xl flex items-center justify-center shrink-0 border border-green-100 shadow-sm">
                        @php
                            $catConfig = $categories[$category] ?? null;
                        @endphp
                        @if($catConfig)
                            <x-dynamic-component :component="$catConfig['icon']" class="w-6 h-6 {{ $catConfig['color'] }}" />
                        @endif
                    </div>
                    <div>
                        <h1 class="text-xl sm:text-2xl font-black text-slate-900">Frases de {{ $categoryName }}</h1>
                        <p class="text-slate-500 text-xs sm:text-sm leading-relaxed">
                            Mostrando as melhores frases curtas da categoria <strong class="text-primary">{{ $categoryName }}</strong> para seu status e postagens.
                        </p>
                    </div>
                </div>
                <a href="{{ route('phrases.create') }}"
                   class="bg-slate-100 border border-slate-200 text-slate-700 hover:text-primary hover:border-primary px-4 py-2.5 rounded-xl text-xs font-bold transition-all shadow-sm flex items-center gap-1">
        ...
        @if ($phrases->isEmpty())
            <div class="bg-white rounded-2xl border border-slate-200 p-12 text-center shadow-sm flex flex-col items-center">
                ...
                <h3 class="text-slate-800 font-bold text-lg mb-1">Nenhuma frase nesta categoria ainda</h3>
                <p class="text-slate-500 text-sm mb-4">Seja o primeiro a enviar uma frase inspiradora para esta categoria!</p>
                <a href="{{ route('phrases.create') }}" class="btn-primary flex items-center gap-1">
                    <x-heroicon-o-plus-circle class="w-4 h-4 text-white" />
                    <span>Enviar Frase Agora</span>
                </a>
            </div>
        @else
