...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-7 h-7 sm:w-8 sm:h-8">
  <path fill-rule="evenodd" d="M12 2a10 10 0 1 0 0 20A10 10 0 0 0 12 2Zm0 5a1 1 0 0 1 1 1v3h3a1 1 0 1 1 0 2h-3v3a1 1 0 1 1-2 0v-3H8a1 1 0 1 1 0-2h3V8a1 1 0 0 1 1-1Z" clip-rule="evenodd"/>
</svg>
@endsection
...
<a href="{{ route('tools.raffle-generator') }}" class="flex flex-col items-center justify-center w-16 h-16 rounded-lg text-slate-600 hover:text-[#25D366] hover:bg-green-50 transition-all {{ request()->is('ferramentas/gerador-de-sorteios*') ? 'text-[#25D366] bg-green-50' : '' }}">
...
<a href="{{ route('tools.raffle-generator') }}" class="flex flex-col items-center justify-center -translate-y-4 relative">
...
<div class="py-8 md:py-12" x-data="raffleGenerator()">
  ...
  <div class="text-center max-w-2xl mx-auto mb-10">
    <div class="inline-flex items-center justify-center w-20 h-20 rounded-[24px] bg-orange-50 border-2 border-orange-100 text-orange-600 mb-6 shadow-[0_0_25px_rgba(234,88,12,0.2)] rotate-3">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-10 h-10 -rotate-3">
        <path fill-rule="evenodd" d="M12 2a10 10 0 1 0 0 20A10 10 0 0 0 12 2Zm-1 6a1 1 0 0 1 2 0v3h3a1 1 0 1 1 0 2h-3v3a1 1 0 1 1-2 0v-3H8a1 1 0 1 1 0-2h3V8Z" clip-rule="evenodd"/>
      </svg>
    </div>
    <h1 class="text-4xl md:text-5xl font-black text-slate-800 mb-4 tracking-tight">
      Gerador de <span class="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-orange-600">Sorteios</span>
    </h1>
    <p class="text-slate-500 text-lg">Cole a lista de membros do grupo, escolha a quantidade de vencedores e sorteie em segundos — funciona com <strong>200+ participantes</strong>.</p>
  </div>
  ...
  <div class="max-w-[1100px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
    ...
    <div class="lg:col-span-5 space-y-5">
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <h2 class="text-lg font-bold text-slate-800 mb-1 flex items-center gap-2">
          <span class="w-6 h-6 rounded-full bg-emerald-600 text-white text-xs font-black flex items-center justify-center flex-shrink-0">1</span>
          Importar Participantes
        </h2>
        <p class="text-xs text-slate-400 font-medium mb-4 ml-8">Cole um nome por linha ou separe por vírgula. Funciona com 200+ nomes.</p>

        {{-- Título do Sorteio --}}
        <div class="mb-5">
          <label class="block text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Nome do Sorteio (Opcional)</label>
          <input type="text" x-model="drawTitle" placeholder="Ex: Sorteio Camisa Oficial" class="w-full px-3 py-2 rounded-xl border border-slate-200 focus:ring-emerald-500 focus:border-emerald-500 font-medium text-slate-700 text-sm outline-none transition-colors">
        </div>
        ...
        <div class="flex flex-wrap gap-1.5 mb-3 bg-slate-100 rounded-xl p-1">
          ...
          <label :class="inputMode === 'csv' ? 'bg-emerald-600 shadow text-white' : 'text-slate-500 hover:text-slate-700 bg-slate-200/50 cursor-pointer'"
          ...
          <div class="space-y-2 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
            <template x-for="(item, index) in items" :key="index">
              <div class="flex items-center gap-2">
                <span class="flex-shrink-0 w-6 h-6 rounded-full bg-emerald-100 text-emerald-700 text-[10px] font-black flex items-center justify-center" x-text="index + 1"></span>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <h2 class="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
          <span class="w-6 h-6 rounded-full bg-emerald-600 text-white text-xs font-black flex items-center justify-center flex-shrink-0">2</span>
          Configurar Sorteio
        </h2>
        ...
        <div class="mt-4 flex flex-col gap-3">
          <label class="flex items-center gap-3 p-3 rounded-xl border border-slate-200 cursor-pointer hover:bg-slate-50 transition-colors">
            <input type="checkbox" x-model="allowRepeats" class="w-4 h-4 text-emerald-600 rounded border-slate-300 focus:ring-emerald-500">
            <span class="text-sm font-medium text-slate-700">Permitir repetição de vencedor</span>
          </label>
          <label class="flex items-center gap-3 p-3 rounded-xl border border-orange-200 bg-orange-50/50 cursor-pointer hover:bg-orange-50 transition-colors">
            <input type="checkbox" x-model="saveDraw" class="w-4 h-4 text-orange-600 rounded border-orange-300 focus:ring-orange-500">
          ...
          <label class="flex items-center gap-3 cursor-pointer group p-2.5 bg-slate-50 rounded-xl border border-slate-100 hover:border-slate-200 transition-colors">
            <input type="checkbox" x-model="ignoreDuplicates" class="w-4 h-4 text-emerald-600 rounded border-slate-300 focus:ring-emerald-500">
      ...
      <button @click="startDraw()" :disabled="spinning || validItems.length < 2"
        ...
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-7 h-7" :class="spinning ? 'animate-spin' : ''">
          <path fill-rule="evenodd" d="M12 2a10 10 0 1 0 0 20A10 10 0 0 0 12 2Zm-1 6a1 1 0 0 1 2 0v3h3a1 1 0 1 1 0 2h-3v3a1 1 0 1 1-2 0v-3H8a1 1 0 1 1 0-2h3V8Z" clip-rule="evenodd"/>
        </svg>
        <span x-text="spinning ? 'Sorteando...' : (validItems.length < 2 ? 'Adicione participantes ↑' : '🎲 Iniciar Sorteio')"></span>
      </button>

      {{-- Info --}}
      <div x-show="validItems.length >= 2" class="flex items-center gap-2 justify-center text-xs text-slate-400 font-medium">
    ...
    <div class="lg:col-span-7 space-y-5">
      ...
      <div x-show="mode === 'roulette'" class="bg-slate-900 rounded-2xl shadow-xl p-6 relative overflow-hidden">
        <div class="absolute top-0 right-0 w-48 h-48 bg-emerald-700/20 rounded-full blur-3xl -mr-16 -mt-16 pointer-events-none"></div>
        <div class="absolute bottom-0 left-0 w-48 h-48 bg-teal-800/20 rounded-full blur-3xl -ml-16 -mb-16 pointer-events-none"></div>
        ...
        <h2 class="text-base font-bold text-white mb-4 flex items-center gap-2 relative z-10">
          ...
          <span class="ml-auto text-xs bg-emerald-900/60 text-emerald-300 font-bold px-2 py-1 rounded-full" x-text="validItems.length + ' nomes'"></span>
        </h2>

        <div class="flex items-center justify-center relative z-10">
          <div class="relative">
            <div class="absolute top-1/2 -right-2 -translate-y-1/2 z-20 drop-shadow-lg">
              <div class="w-0 h-0 border-y-[14px] border-y-transparent border-l-[22px] border-l-white filter drop-shadow-md"></div>
            </div>
        ...
        <div x-show="result.length > 0 && !spinning" class="mt-5 relative z-10"
          ...
          <div class="bg-gradient-to-r from-emerald-900/60 to-teal-900/60 border border-emerald-700/50 rounded-2xl p-4 text-center shadow-lg">
            ...
            <div x-show="currentDrawUrl" class="mt-3 pt-3 border-t border-emerald-800/50">
              <p class="text-[10px] text-slate-400 font-medium mb-1">🔗 Link Oficial do Sorteio (Copiado automaticamente):</p>
              <a :href="currentDrawUrl" target="_blank" class="text-xs font-bold text-emerald-400 hover:text-emerald-300 transition-colors break-all" x-text="currentDrawUrl"></a>
            </div>
          </div>
        </div>
        ...
        <div class="relative z-10">
          ...
          <template x-if="!spinning && shuffledAll.length === 0 && validItems.length < 2">
            <div class="flex flex-col items-center justify-center h-48 text-slate-600">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor" class="w-14 h-14 mb-3 opacity-30">
          ...
          <template x-if="!spinning && shuffledAll.length === 0 && validItems.length >= 2">
            <div class="flex flex-col items-center justify-center h-48 text-slate-500">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-14 h-14 mb-3 opacity-30">
              ...
              <p class="text-sm font-bold" x-text="validItems.length + ' participantes prontos. Clique em Sortear!'"></p>
            </div>
          </template>
          ...
          <template x-if="spinning && mode === 'random'">
            <div class="space-y-2">
              <template x-for="n in Math.min(6, validItems.length)" :key="n">
                <div class="h-10 bg-emerald-900/30 rounded-xl animate-pulse" :style="'animation-delay:' + (n * 80) + 'ms'"></div>
              </template>
            </div>
          </template>
          ...
          <template x-if="!spinning && shuffledAll.length > 0">
            <div>
              ...
              <div class="mb-4 space-y-2">
                <div x-show="currentDrawUrl" class="flex items-center justify-center gap-2 mb-3 px-3 py-2 bg-emerald-900/40 border border-emerald-800/50 rounded-lg">
                   <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4 text-emerald-400"><path fill-rule="evenodd" d="M10 1a4.5 4.5 0 00-4.5 4.5V9H5a2 2 0 00-2 2v6a2 2 0 002 2h10a2 2 0 002-2v-6a2 2 0 00-2-2h-.5V5.5A4.5 4.5 0 0010 1zm3 8V5.5a3 3 0 10-6 0V9h6z" clip-rule="evenodd" /></svg>
                   <span class="text-[10px] font-black text-emerald-300 uppercase tracking-widest">Verificado e Registrado</span>
                </div>
                <template x-for="(item, idx) in shuffledAll.slice(0, winners)" :key="'w' + idx">
                  <div class="flex items-center gap-3 px-4 py-3 rounded-xl bg-gradient-to-r from-emerald-700/60 to-teal-700/60 border border-emerald-600/50"
                    x-transition:enter="transition ease-out duration-300"
                    :style="'transition-delay:' + (idx * 60) + 'ms'">
                    <span class="text-xl" x-text="idx === 0 ? '🥇' : (idx === 1 ? '🥈' : (idx === 2 ? '🥉' : '🏅'))"></span>
                    <span class="font-black text-white flex-1 text-base" x-text="item"></span>
                    <span class="text-[10px] font-black text-emerald-300 bg-emerald-900/40 px-2 py-0.5 rounded-full" x-text="(idx + 1) + 'º lugar'"></span>
                  </div>
                </template>
              </div>
              ...
              <template x-if="shuffledAll.length > winners">
                <div>
                  <button @click="showAll = !showAll" class="w-full text-xs text-slate-500 hover:text-slate-300 font-bold py-2 flex items-center justify-center gap-1 transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4 transition-transform" :class="showAll ? 'rotate-180' : ''">
                      <path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 0 1 1.06.02L10 11.168l3.71-3.938a.75.75 0 1 1 1.08 1.04l-4.25 4.5a.75.75 0 0 1-1.08 0l-4.25-4.5a.75.75 0 0 1 .02-1.06Z" clip-rule="evenodd"/>
                    </svg>
                    <span x-text="showAll ? 'Ocultar demais' : 'Ver todos os ' + shuffledAll.length + ' participantes na ordem'"></span>
                  </button>
                  <div x-show="showAll" x-transition class="space-y-1 max-h-64 overflow-y-auto custom-scrollbar mt-2">
                    <template x-for="(item, idx) in shuffledAll.slice(winners)" :key="'o' + idx">
                      <div class="flex items-center gap-2.5 px-3 py-2 rounded-lg bg-slate-800/60">
                        <span class="w-6 h-6 rounded-full bg-slate-700 text-slate-400 text-[10px] font-black flex items-center justify-center flex-shrink-0" x-text="winners + idx + 1"></span>
      ...
      <div x-show="result.length > 0 && !spinning" class="flex flex-col sm:flex-row gap-3"
        ...
        <button @click="startDraw()" :disabled="spinning"
          ...
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5"><path fill-rule="evenodd" d="M15.312 11.424a5.5 5.5 0 0 1-9.201 2.466l-.312-.311h2.433a.75.75 0 0 0 0-1.5H3.989a.75.75 0 0 0-.75.75v4.242a.75.75 0 0 0 1.5 0v-2.43l.31.31a7 7 0 0 0 11.712-3.138.75.75 0 0 0-1.449-.39Zm1.23-3.723a.75.75 0 0 0 .219-.53V2.929a.75.75 0 0 0-1.5 0V5.36l-.31-.31A7 7 0 0 0 3.239 8.188a.75.75 0 1 0 1.448.389A5.5 5.5 0 0 1 13.89 6.11l.311.31h-2.432a.75.75 0 0 0 0 1.5h4.243a.75.75 0 0 0 .53-.219Z" clip-rule="evenodd"/></svg>
          Sortear Novamente
        </button>
      ...
      <div class="space-y-4">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-5">
          ...
          <p class="text-xs text-slate-500 mb-3">Possui o código de um sorteio? Digite-o para ver o resultado oficial.</p>
          <form action="{{ route('tools.raffle.search') }}" method="GET" class="flex gap-2">
            <input type="text" name="code" placeholder="Ex: 9E8F8B2C" required class="flex-1 px-3 py-2 rounded-xl border border-slate-200 focus:ring-orange-500 focus:border-orange-500 font-mono text-sm outline-none transition-colors uppercase">
            <button type="submit" class="bg-slate-800 hover:bg-slate-700 text-white px-4 py-2 rounded-xl font-bold text-xs transition-colors shadow-sm">Buscar</button>
          </form>
          @if(session('error'))
        ...
        <div x-show="history.length > 0" class="bg-white rounded-2xl shadow-sm border border-slate-200 p-5">
          ...
          <span class="text-[10px] text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md" x-text="history.length + ' salvo(s)'"></span>
        </h3>
        <div class="space-y-3 max-h-64 overflow-y-auto pr-1 custom-scrollbar">
          <template x-for="(entry, idx) in history.slice().reverse()" :key="idx">
            <div class="border border-slate-100 bg-slate-50/50 rounded-xl p-3 flex flex-col gap-2 transition-colors hover:bg-slate-50">
              <div class="flex justify-between items-start gap-2">
                <div class="flex-1">
                  <p class="text-xs font-black text-slate-800 mb-0.5" x-text="entry.title"></p>
                  <p class="text-[10px] text-slate-500 font-medium font-mono">ID: <span x-text="entry.hash"></span></p>
                </div>
                <span class="text-[10px] font-bold text-slate-400 bg-white border border-slate-200 px-1.5 py-0.5 rounded shadow-sm shrink-0" x-text="entry.time"></span>
              </div>
              
              <div class="flex items-center gap-2 text-xs font-bold text-emerald-700 bg-emerald-50/50 rounded-lg p-2 border border-emerald-100/50">
              ...
              <div class="flex items-center justify-between mt-1">
                ...
                <a x-show="entry.url" :href="entry.url" target="_blank" class="text-[10px] font-black text-blue-600 hover:text-blue-800 flex items-center gap-1 transition-colors">
                  Ver Oficial <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-3 h-3"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 6H5.25A2.25 2.25 0 0 0 3 8.25v10.5A2.25 2.25 0 0 0 5.25 21h10.5A2.25 2.25 0 0 0 18 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" /></svg>
                </a>
              </div>
            </div>
          </template>
