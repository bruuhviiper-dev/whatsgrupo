...
<div class="card p-5 mb-4 dark:bg-slate-800 dark:border-slate-700">
    <div class="flex items-center justify-between mb-3">
        <div>
            <h3 class="font-bold text-slate-900 dark:text-white text-sm mb-0.5">Google Analytics (GA4)</h3>
            <p class="text-slate-400 dark:text-slate-500 text-xs">Mede o tráfego em todas as páginas do site.</p>
        </div>
        <label class="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" name="ga_enabled" value="1" class="sr-only peer"
                {{ $settings['ga_enabled'] == '1' ? 'checked' : '' }}>
            <div class="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer dark:bg-slate-700 peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-emerald-500"></div>
        </label>
    </div>

    <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">ID de medição (Measurement ID)</label>
    <p class="text-slate-400 dark:text-slate-500 text-xs mb-2">Ex: <code class="bg-slate-100 dark:bg-slate-700 dark:text-slate-300 px-1.5 py-0.5 rounded text-[10px] font-mono">G-XXXXXXXXXX</code>. Geramos o gtag.js automaticamente.</p>
    <input type="text" name="ga_measurement_id"
           value="{{ old('ga_measurement_id', $settings['ga_measurement_id']) }}"
           placeholder="G-XXXXXXXXXX"
           class="w-full border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2.5 text-sm text-slate-800 dark:text-white bg-slate-50 dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-400/50 focus:border-emerald-400 transition mb-4">

    <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Ou cole o script completo do Google Analytics</label>
    <p class="text-slate-400 dark:text-slate-500 text-xs mb-2">Opcional. Se preenchido, tem prioridade sobre o ID acima (cole o trecho <code class="bg-slate-100 dark:bg-slate-700 dark:text-slate-300 px-1.5 py-0.5 rounded text-[10px] font-mono">&lt;script async src="...gtag/js?id=..."&gt;</code>).</p>
    <textarea name="ga_script" rows="5"
              placeholder='<!-- Google tag (gtag.js) --><script async src="https://www.googletagmanager.com/gtag/js?id=G-XXXXXXXXXX"></script>...'
              class="w-full border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2.5 text-xs font-mono text-slate-800 dark:text-white bg-slate-50 dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-400/50 focus:border-emerald-400 transition resize-none">{{ old('ga_script', $settings['ga_script']) }}</textarea>
</div>
