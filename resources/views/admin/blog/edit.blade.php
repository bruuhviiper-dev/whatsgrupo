...
<div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden p-6">
    <form action="{{ route('admin.blog.update', $post->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" value="{{ old('title', $post->title) }}" required class="w-full px-4 py-2 border border-slate-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500">
        ...
        <div class="mb-4" x-ignore>
            ...
            <textarea id="content" name="content" class="w-full px-4 py-2 border border-slate-300 rounded-lg shadow-sm min-h-[300px]">{{ old('content', $post->content) }}</textarea>
            @error('content') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
        </div>

        <div class="mb-4">
            <label class="block text-sm font-bold text-slate-700 mb-1">Meta Description (SEO)</label>
            <input type="text" name="meta_description" value="{{ old('meta_description', $post->meta_description) }}" maxlength="255" class="w-full px-4 py-2 border border-slate-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500">
        </div>

        <div class="mb-6 flex items-center">
            <input type="hidden" name="is_published" value="0">
            <input type="checkbox" name="is_published" id="is_published" value="1" {{ old('is_published', $post->is_published) ? 'checked' : '' }} class="rounded border-slate-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
            <label for="is_published" class="ml-2 text-sm font-bold text-slate-700">Publicado</label>
        </div>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg font-bold hover:bg-blue-700">Atualizar Notícia</button>
        </div>
    </form>
